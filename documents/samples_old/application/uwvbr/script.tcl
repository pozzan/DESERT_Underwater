...
set opt(vbr_period)	 		5
set opt(send_period)		20
set opt(starttime)       	1
set opt(stoptime)        	1000 ;# Simulation end in seconds
...
if {$argc != 6} {
  ...
  puts "- the first for the vbr seed;"
  puts "- the second one is the seed of the positions;"
  puts "- the third one is the vbr poisson period;"
  puts "- the fourth one is the the path estimation poisson period."
  puts "For example ns SpreadUW_test.tcl 4 2 100 10"
  puts "Please try again."
  return
} else {
	set opt(seedvbr)		[lindex $argv 0]
	set opt(seedposition)	[lindex $argv 1]
	set opt(period1)        [lindex $argv 2]
	set opt(period2)        [lindex $argv 3]
	set opt(timer_switch_1) [lindex $argv 4]
	set opt(timer_switch_2) [lindex $argv 5]
...
Module/UW/VBR set period1_             $opt(period1)
Module/UW/VBR set period2_             $opt(period2)
...
# Attention: timer_ack_waiting_ < opt(vbr_period), and timer_ack_waiting_ depends on the distance between nodes
...
proc createNode { id } {
    ...
    global channel propagation data_mask ns vbr position node udp portnum ipr ipif channel_estimator
...
proc finish {} {
	...
	if ($opt(verbose)) {
		...
		puts "vbr period 1     : $opt(period1) s"
		puts "vbr period 2     : $opt(period2) s"
		puts "switch period 1  : $opt(timer_switch_1) s"
		puts "switch period 2  : $opt(timer_switch_2) s"
		puts "period  send     : $opt(send_period) ms"
		puts "seed vbr         : $opt(seedvbr)"
	...
	set usefuldata          [expr $sum_vbr_rcv_pkts * $opt(pktsize)]
	...
	if ($opt(verbose)) {
		...
		puts [format "lambda           	 : %.3f" [expr 60.0/$opt(vbr_period)]]
		puts "done!"
	}
	if ($opt(fileoutput)) {
		puts $opt(outfile) [format "%s	%s	%s	%s" "$sum_vbr_throughput" "[expr 1 - $sum_vbr_rcv_pkts / $sum_vbr_sent_pkts]" "$overhead" "$totalenergy"]
	}
