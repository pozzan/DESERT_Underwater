...
if {$argc != 4} {
  ...
  puts "For example ns script.tcl 4 2 100 10"
  ...
  return
	...
	set opt(buffer_period)  [lindex $argv 3]
...
# Important: period_data_ + timer_buffer_ > timer_ack_waiting_
# TEST: ns sun_.tcl 0 0 600 1 10, buf 5, timer buff 7 (4 nodes)
...
Module/UW/SUNNode set timer_route_validity_	    360000
Module/UW/SUNNode set timer_sink_probe_validity_ 250
Module/UW/SUNNode set printDebug_				0
Module/UW/SUNNode set setAckMode_				$opt(ackmode)
Module/UW/SUNNode set max_ack_error_			3
Module/UW/SUNNode set timer_ack_waiting_		8; # Actually this value it is not used
Module/UW/SUNNode set metrics_					$opt(metric)
Module/UW/SUNNode set alpha_					2.0/3.0
Module/UW/SUNNode set probe_min_snr_            15
Module/UW/SUNNode set buffer_max_size_          5
Module/UW/SUNNode set timer_buffer_             8
...
Module/UW/SUNSink set setAckMode_				$opt(ackmode)
...
for {set id 0} {$id < $opt(nn)} {incr id}  {
    createNode $id
	$ipr($id) initialize ;#The initialize is used by the sun module to get the IP from the ip module
}
...
proc finish {} {
	...
	if ($opt(verbose)) {
		...
		puts "buffer period    : $opt(buffer_period) s"
	...
	for {set i 0} {$i < $opt(nn)} {incr i}  {
		...
		set cbr_throughput_		[expr double($cbr_rcv_pkts * $opt(pktsize) * 8) / double($opt(txduration))]
		...
		if ($opt(verbose)) {
			puts "cbr_node($i) throughput	: $cbr_throughput"
			puts "cbr_node($i) throughput_	: $cbr_throughput_ bit/s"
			puts "cbr_node($i) per			: $cbr_per"
			puts "cbr_node($i) sent:		-> $cbr_sent_pkts"
			puts "cbr_node($i) received:		<- $cbr_rcv_pkts"
		}
		
		set sum_cbr_throughput [expr $sum_cbr_throughput + $cbr_throughput_]
	...
	if ($opt(verbose)) {
		puts ""
		puts "Statistics: Packets sent and packets received (SUN)"
		for {set i 0} {$i < $opt(nn)} {incr i}  {
			puts "sun_node ($i) sent:	$node_stats($i)"
			puts "sun_node ($i) recv:	$sink_stats($i)"
		}
		puts ""
		puts "Node: the number of packets generated by the cbr module is always >= to the"
		puts "number of data packets sent by the SUN module (the buffer can be full)."
	}
	...
	set totaldata           [expr $probecount*($probeheadersize + $ipheadersize) + ($ackcountnode+$ackcountsink)*($ackheadersize + $ipheadersize) + $pathestcount*($pathestheadersize + $ipheadersize) + $datacount*($dataheadersize + $opt(pktsize) + $udpheadersize + $ipheadersize)]
	set totaldata_clean     [expr $probecount*($probeheadersize + $ipheadersize) + ($ackcountnode+$ackcountsink)*($ackheadersize + $ipheadersize) + $pathestcount*($pathestheadersize + $ipheadersize) + ($datacount - $forwardcount)*($dataheadersize + $opt(pktsize) + $udpheadersize + $ipheadersize)]
	set usefuldata          [expr $sum_cbr_rcv_pkts * $opt(pktsize)]
	set overhead            [expr 1 - $usefuldata/$totaldata]
	set overhead_clean      [expr 1 - $usefuldata/$totaldata_clean]
	...
	if ($opt(verbose)) {
		...
		puts "Number of forwarded pkts : $forwardcount"
		...
		puts "Overhead (without retx)  : $overhead_clean"
