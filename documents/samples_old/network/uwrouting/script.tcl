...
# Area of Simulation  #
#######################
set opt(x_length)			10000     ;#In meters.
...
################################
if {$argc != 4} {
  puts "The script_with_cube.tcl script requires three numbers to be inputed:"
...
Module/UW/SUWNode set PoissonTraffic_			1
Module/UW/SUWNode set periodPoissonTraffic_	[lindex $argv 3]
Module/UW/SUWNode set timer_route_validity_	720
Module/UW/SUWNode set timer_sink_probe_validity_ 220
Module/UW/SUWNode set printDebug_				0
Module/UW/SUWNode set setAckMode_				$opt(ackmode)
Module/UW/SUWNode set max_ack_error_			2
Module/UW/SUWNode set timer_ack_waiting_		30
Module/UW/SUWNode set metrics_					1
Module/UW/SUWNode set alpha_					2.0/3.0

Module/UW/SUWSink set PoissonTraffic_			1
Module/UW/SUWSink set periodPoissonTraffic_	[lindex $argv 3]
Module/UW/SUWSink set printDebug_ 				0
Module/UW/SUWSink set t_probe					60

# Mobility model configuration
# Gauss Markov
set opt(speedMean) 1
Position/GM set xFieldWidth_	10000
Position/GM set yFieldWidth_	5000
Position/GM set yFieldWidth_	200
Position/GM set alpha_			0.90
Position/GM set alphaPitch_		0.90
Position/GM set directionMean_	0
Position/GM set pitchMean_		0
Position/GM set sigmaPitch_		0
Position/GM set updateTime_		5
Position/GM set debug_			0

# DRIFT
Position/DRIFT set boundx_               1;      # 0 no bounds, 1 yes
Position/DRIFT set boundy_               1
Position/DRIFT set boundz_               1
Position/DRIFT set xFieldWidth_	         100000;  # The module uses these values only if the corrispective bound is set to 1, in meters
Position/DRIFT set yFieldWidth_	         2000
Position/DRIFT set zFieldWidth_	         200
Position/DRIFT set speed_horizontal_     1;      # Horizontal component of the speed, in m/s. 0 means no constant horizontal speed = completely random
Position/DRIFT set speed_longitudinal_   0
Position/DRIFT set speed_vertical_       0
Position/DRIFT set alpha_	             0.7
Position/DRIFT set deltax_               1      
Position/DRIFT set deltay_               1
Position/DRIFT set deltaz_               1
Position/DRIFT set starting_speed_x_     0
Position/DRIFT set starting_speed_y_     0
Position/DRIFT set starting_speed_z_     0
Position/DRIFT set updateTime_		     2;      # In seconds
Position/DRIFT set debug_			     0
...
proc createNode { id } {
    ...
    set ipr($id)  [new Module/UW/SUWNode]
    ...
    if {$opt(mobility)} {
		if {$opt(gm)} {
			set position($id) [new "Position/GM"]
		} else {
			set position($id) [new "Position/DRIFT"]
		}
	} else {
...
proc createSink { } {
	...
	set ipr_sink       [new Module/UW/SUWSink]
	...
	if {$opt(mobility)} {
		if {$opt(gm)} {
			set position_sink [new "Position/GM"]
		} else {
			set position_sink [new "Position/DRIFT"]
		}
	} else {
	...
	$position_sink setX_ $curr_x
	$position_sink setY_ $curr_y
...
for {set id 0} {$id < $opt(nn)} {incr id}  {
    createNode $id
	$ipr($id) initialize ;#The initialize is used by the spreaduw module to get the IP from the ip module
}
...
proc finish {} {
	...
	set tmp_node_stats(0) "0 0 0 0 0"
	if ($opt(verbose)) {
		puts ""
		puts "Thorughput"
	}
	for {set i 0} {$i < $opt(nn)} {incr i}  {
		...
		#set txbyte				[$phy($i) getTxByte]
		#set rxbyte				[$phy($i) getRxByte]
		for {set j 0} {$j < 6} {incr j} { ;#TODO: fix this value 4
			set tmp_node_stats($j)	[$ipr($i) getstats $j]
		}
		set node_stats($i) "$tmp_node_stats(0) $tmp_node_stats(1) $tmp_node_stats(2) $tmp_node_stats(3) $tmp_node_stats(4) $tmp_node_stats(5)"
	...
	#Add the energy consumed by the sink. TODO: fix this for multiple sinks
	...
	for {set i 0} {$i < $opt(nn)} {incr i}  {
		...
		set sink_stats($i) "$tmp_sink_stats(0) $tmp_sink_stats(1) $tmp_sink_stats(2) $tmp_sink_stats(3) $tmp_sink_stats(4) $tmp_sink_stats(5)"
	}
	if ($opt(verbose)) {
		puts ""
		puts "Statistics: Packets sent (s) and packets received (r)"
		for {set i 0} {$i < $opt(nn)} {incr i}  {
			puts "s:	$node_stats($i)"
			puts "r:	$sink_stats($i)"
		}
	...
	set ackcount            [$ipr(1) getackcount]
	set datacount			[$ipr(1) getdatapktcount]
	set datadrop			[$ipr(1) getdatapktdropped]
	set pathestcount		[$ipr(1) getpathestablishmentpktcount]
	set ipcount             [expr $probecount + $ackcount + $pathestcount]
	set probeheadersize		[$ipr_sink getprobepktheadersize]
	set ackheadersize       [$ipr(1) getackheadersize]
	set dataheadersize      [$ipr(1) getdatapktheadersize]
	set pathestheadersize   [$ipr(1) getpathestheadersize]
	set ipheadersize        [$ipif(1) getipheadersize]
	set udpheadersize       [$udp(1) getudpheadersize]
	set cbrheadersize       [$cbr(1) getcbrheadersize]
	set totaldata           [expr $probecount*$probeheadersize + $ackcount*$ackheadersize + $pathestcount*$pathestheadersize + $ipcount*$ipheadersize + $datacount*($dataheadersize + $opt(pktsize) + $udpheadersize + $ipheadersize)]
	set usefuldata          [expr $sum_cbr_rcv_pkts * $opt(pktsize)]
	set overhead            [expr 1 - $usefuldata/$totaldata]
	
	if ($opt(verbose)) {
		puts ""
		puts "Other statistics"
		...
		puts "Number of ack pkts       : $ackcount"
		puts "Number of data pkts      : $datacount"
		puts "Number of data pkts drop : $datadrop"
		...
		puts "Total Bytes sent         : $sum_txbyte"
		puts "Total Bytes received     : $sum_rxbyte"
