...
clear;
close all;
fileID = fopen('path_matlab_def.csv');
...
while(index_sup_t<numel(x_t) && index_sup_res<numel(x_res))
   ...
   distance_vector=[distance_vector,distanceVector(x_t(index_inf_t:index_sup_t),y_t(index_inf_t:index_sup_t),z_t(index_inf_t:index_sup_t),x_res(index_inf_res:mom_index_res),y_res(index_inf_res:mom_index_res),z_res(index_inf_res:mom_index_res))];
%    if(k==10)
...
distance_vector=distanceVector(x_t,y_t,z_t,x_res,y_res,z_res);
mse=mean(MSE)
figure;
plot(x_res,distance_vector)